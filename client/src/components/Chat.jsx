...
const Chat = ({userName}) => {
  ...
  const handleSearch = (userName) => {
    if(userName.length > 0){
      const searchResults = users.filter((item) => item.firstName.includes(userName))
      setSearchedUsers(searchResults)
      console.log(searchResults)
      return
    }
    const searchResults = users.filter((item) => item.firstName.includes(keyword))
    console.log(searchResults)
    setSearchedUsers(searchResults)
  }
  useEffect(()=>{
  ...
  return (
    ...
    <div className='font-montserrat bg-white flex flex-col'>
        <div className='flex justify-around w-full mt-2 mb-12'>
          <input className=' w-1/2 mx-6 px-4 py-1 text-xl placeholder:text-xl 
          border-b-2 border-black
          focus:outline-none' 
          placeholder='Search keyword...'
          onChange={(e) => setKeyword(e.target.value)}/>
          <button className='mx-6 py-2 px-14 text-2xl font-semibold border border-black
          text-white bg-black
          focus:outline-none'
    ...
    <div className='grow rounded-b-sm border-2 flex border-gray-600
    '>
